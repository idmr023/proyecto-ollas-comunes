...
export default function OfflineBanner() {
  ...
  // Estados previos para descolapsar reactivamente
  ...
  // Descolapsar reactivamente ante cambios críticos
  useEffect(() => {
  ...
  // 1. Caso: Online con conflictos sin cambios pendientes
  ...
  // 2. Caso: Online con sincronizaciones en progreso
  ...
  // Si estamos online y no hay pendientes de sincronizar, no se muestra nada
  if (online && pendingCount === 0) return null

  // 3. Caso: Offline activo (!online)
  if (isCollapsed) {
    return collapsedPill(
      "bg-amber-500/90 dark:bg-amber-600/90 backdrop-blur-md text-amber-950 dark:text-amber-50",
      <WifiOff className="h-4.5 w-4.5 text-amber-950 dark:text-amber-50 animate-pulse" />,
