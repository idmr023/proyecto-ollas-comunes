...
class ControllerCalculo extends Notifier<EstadoCalculo> {
  ...
  Future<void> calcular({required String recetaId, int? personas}) async {
    ...
    final Resultado<ResultadoPreparacion> resultado = await _repositorio
        .calcular(recetaId: recetaId, personas: personas);
    state = switch (resultado) {
      Exito<ResultadoPreparacion>(:final ResultadoPreparacion valor) =>
        EstadoCalculo.exito(valor),
      Fallo<ResultadoPreparacion>(:final excepcion) => EstadoCalculo.error(
        excepcion.mensaje,
      ),
    };
  }
...
final NotifierProvider<ControllerCalculo, EstadoCalculo>
controllerCalculoProvider = NotifierProvider<ControllerCalculo, EstadoCalculo>(
  ControllerCalculo.new,
);
